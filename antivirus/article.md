...
Откроется привычное окно эмулятора терминала, только с зелёной рамкой окна.
...
меня
https://www.clamav.net/ не открывается, как будто заблокирован)
...
Для удобства добавим в */etc/bash.bashrc* (тут много вариаций допустимы, мне было проще сделать именно
так)
...
Всё, шаблон готов. Но нас ждёт облом с обновлениями. Среди пакетов debian есть собственно 
антивирус, а сигнатур нет. Обновление (для первого раза - скачивание) сигнатур - процедура 
обязательная перед сканированием. Иначе ни демон, ни однопоточный вариант ClamAV просто 
откажется работать.  
...
Ох, не зря я всё
подряд скачиваю и сохраняю на локальном диске :) 
...
Отрубаем выход в сеть. Для "песочницы" Интернет не нужен.
...
Откроется окно эмулятора терминала
