...
хранилищах и т.п. На серверах, одним словом. На настольных компьютерах как-то не сложилось... А
возможностей хоть отбавляй! Дозволяется установить антивирус прямо на физическую машину (и даже
включить сканирование «на лету»). Или в Docker-контейнер. Про виртуальную машину говорить нечего. Но
всё это скучно. Хочется чего-нибудь эдакого. Почему бы не попробовать
...
Официальный сайт антивируса ClamAV ([www.clamav.net](https://www.clamav.net/)) и серверы обновлений
стали недоступны (по крайней мере, из Беларуси). Так что придётся повозиться.
...
Есть в *debian-11* кое-что лишнее. Например, при работе с антивирусом совершенно не нужны
...
именно так) алиас, чтобы не вбивать каждый раз полную команду сканирования вручную. На мой взгляд,
...
штатный механизм обновлений ни о каком ClamAV не знает), или паковать сигнатуры вручную.
...
Теперь об обходе блокировок. Пустить в Qubes OS трафик
[через TOR](https://www.whonix.org/wiki/Qubes) – не проблема. Да и с VPN особых трудностей не
возникает (возможно, напишу статью о своём опыте).
...
Если файл чист, то всё равно закрываем **disp2525**: антивирус нам больше не нужен, а оригинал
остался в кубе *work*.

Почему же мы в «песочнице» сканируем каталог *QubesIncoming* целиком? Потому что перед копированием
одноразовый куб чист, и нет нужды заходить глубже. Так, скопированный из куба *work* файл
...
Штатный механизм обновлений (по крайне мере, в Qubes OS 4.1) не делает `apt autoremove`. Так что
если вдруг вылезла ошибка, что какой-то там куб is running out of storage space, то первое, что
нужно сделать – открыть его терминал и вбить `sudo apt autoremove`. Я так делаю после каждого
обновления системы. На самом деле, через раз или даже через два, ибо каждый раз муторно :)
