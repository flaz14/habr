...
не проблема. Да и с VPN особых трудностей не возникает (я прокладывал туннели, но углубляться в
подробности не буду: по моим наблюдениям, запрограммировать что-либо проще, чем толково рассказать
об этом).

Для статьи будет вполне достаточно подкинуть сигнатуры вручную. У себя в закромах я их нашёл и
...
Зайдём в свойства куба, просто для подстраховки. Нажимаем на кнопку с буквой **Q** (совсем как
кнопка "Пуск" в Windows). Далее *Template (disp): antivirus* → *antivirus: Qube Settings*. 
Откроется окно настроек куба:

![antivirus basic settings.png](antivirus%20basic%20settings.png)
...
Осталось вынести ярлык на рабочий стол. Нажимаем на кнопку с буквой **Q** (совсем как кнопка
"Пуск" в Windows), наводим курсор на пункт *Template (disp): antivirus*, в выпавшем подменю находим
*antivirus: Terminal*, хватаем этом пункт мышью и тащим на рабочий стол.
