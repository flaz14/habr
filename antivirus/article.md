...
невысокими привилегиями. Подход со сканированием в «песочнице» напрашивается сам собой. А
[Qubes OS](https://www.qubes-os.org/), наряду с Docker-контейнерами и полновесными виртуальными
...
[TOR](https://ru.wikipedia.org/wiki/Tor). Возьмём *debian-11* (с таким же успехом можно взять
*fedora-34*). Это шаблон из штатной поставки ОС. В нём нет ничего лишнего. Самое то в качестве
основы.
...
Цвета –
[основа](https://doc.qubes-os.org/en/r4.3/introduction/getting-started.html#color-security)
графического интерфейса Qubes OS. Как правило, самый безопасный куб – чёрного цвета. Самый опасный –
красного. Но в целом выбор цвета произволен. Мы выбрали зелёный, потому что собственно шаблон
антивируса не несёт угрозы. В то же время этот шаблон так или иначе будёт иметь доступ к Интернету в
обход
...
Во-вторых, освободилось 500 мегабайт дискового пространства.
...
ввиду ограничений GitHub'а. Но я подумал: а кому реально нужно старьё? Выложу много небольший
файлов.

Скачиваем и склеиваем:

```bash
cat signatures.tar.01 \
signatures.tar.02 \
signatures.tar.03 \
signatures.tar.04 \
signatures.tar.05 \ 
...
И размещаем в каталоге */var/lib/clamav* шаблона *antivirus-base*, чтобы получилась такая картина:
...
Наконец, превращаем куб в одноразовый:
...
И делаем его видимым из графического интерфейса:
...
Заходим в свойства куба, просто для подстраховки. Нажимаем на кнопку **Q**
...
посредством *apt* не удаляет ярлыки). Их можно удалить вручную, перейдя на вкладку *Applications*:

![antivirus-applications-settings.png](images/antivirus-applications-settings.png)

А на вкладке *Advanced*, можно убедиться, что наш куб является шаблоном одноразовых кубов:
...
Пусть у нас есть куб *work* с выходом в Интернет. Мы побродили по Сети и скачали подозрительный
...
Если файл заражён (EICAR-файл всегда будет выглядеть зараженным, хотя это всего лишь текст), то
просто закрываем окно консоли антивируса. Одноразовый куб тут же уничтожится, а вместе с ним
сотрётся просканированный файл. Останется удалить оригинал в кубе *work*.
...
именем **antivirus** снижает вероятность того, что потенциально заражённый файл попадёт по
недосмотру в неподходящий куб. Плюс экономия времени на запуск, экономия памяти.

С другой, когда все сканирования делаются в одной «песочнице», эта самая песочница перестаёт быть
таковой. То есть если вдруг вирус в сканируемом файле поимеет антивирус, то сможет заразить вообще
всё, что за данный сеанс через антивирус пройдёт.
...
После обновлений Qubes OS (по крайне мере, версия 4.1) не запускает `apt autoremove`. Так что если
