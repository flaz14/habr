...
`LinkedHashSet` элементы добавляются в конец. А в случае с `HashSet` неизвестно куда. Да толку нам от гарантированного
добавления в конец, если нам нужно заменить элемент(ы) в середине коллекции.
...
допустимый случай. Важно вот что: править коллекцию "на лету" нельзя. Вместо процедуры будет функция, которая складывает
обработанные элементы в новую коллекцию.

Итого, требуется написать обобщённую функцию, которая принимает в качестве аргумента коллекцию объектов, обходит её и
...
Пожалуй, цикл проще рекурсии. Рассмотрим 
